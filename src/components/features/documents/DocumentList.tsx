...
const DocumentList: React.FC = () => {
  ...
  const getDocumentIcon = (type: string) => {
    switch (type) {
      case 'image':
        return <Image className="text-primary" size={20} />;
      case 'note':
    ...
    return (
      <div className="space-y-4">
        <Tabs defaultValue="all" onValueChange={setSelectedFolder} className="w-full">
          <TabsList className="w-full flex justify-start overflow-x-auto pb-1 scrollbar-none bg-card">
            {folders.slice(0, 5).map((folder) => (
              <TabsTrigger 
                key={folder.id} 
                value={folder.id}
                className="px-3 py-1.5 text-sm whitespace-nowrap flex-shrink-0 data-[state=active]:bg-primary"
              >
                {folder.name}
              </TabsTrigger>
            ))}
  ...
  return (
    <div className="space-y-6">
      <div className="flex gap-2 overflow-x-auto pb-2 hide-scrollbar">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <button className="py-1.5 px-3 rounded-lg text-sm whitespace-nowrap bg-secondary/50 hover:bg-secondary/80 flex items-center gap-1">
              <Plus size={14} />
              New Folder
            </button>
          </DialogTrigger>
  ...
  return (
    <div className="grid gap-3">
      {documents.map((doc) => (
        <div
          key={doc.id}
          className="flex items-center p-3 bg-card rounded-lg border border-border hover:bg-secondary/20 transition-shadow cursor-pointer"
        >
          <div className="bg-primary bg-opacity-10 p-2 rounded-lg mr-3 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className={cn(
              ...
              <span className="bg-secondary px-2 py-0.5 rounded-full truncate">
                {doc.category}
              </span>
            </div>
          </div>
