...
const TimeGrid: React.FC<TimeGridProps> = ({
  ...
  return <ScrollArea className="overflow-auto" style={{
    height: scrollContainerHeight,
    position: 'relative'
  }} ref={scrollRef}>
      <div className="relative">
        <div className="grid grid-cols-8 divide-x border-gray-800">
          <div className="sticky left-0 z-10 border-r border-gray-800 bg-background" style={{
          minWidth: "5rem"
        }}>
            {hours.map((hour, i) => <div key={`hour-${i}`} className="border-b h-[60px] px-2 py-1 text-right text-xs text-muted-foreground" style={{
          ...
          {daysInWeek.map((day, dayIndex) => <div key={`day-${dayIndex}`} className={cn("relative", isToday(day) ? "bg-accent/10" : "", isWeekend(day) ? "bg-muted/5" : "")}>
              {hours.map((_, hourIndex) => <div key={`${dayIndex}-${hourIndex}`} className="border-b relative" style={{
            height: `${minCellHeight}px`
          }}>
            ...
            return group.map((event, eventIndex) => <div key={`event-${dayIndex}-${groupIndex}-${eventIndex}`} className={cn("absolute rounded-sm text-xs p-1 overflow-hidden cursor-pointer", "hover:opacity-90 transition-opacity touch-manipulation")} style={getMultiHourEventStyle(event, day, group.length, eventIndex)} onClick={() => handleViewEvent(event)}>
                    ...
                    {event.location && <div className="flex items-center text-white/90 text-[10px]">
