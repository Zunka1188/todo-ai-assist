...
const initialEvents: Event[] = [{
  ...
  startDate: new Date(2025, 3, 15, 19, 0),
  // April 15, 2025, 7:00 PM
  endDate: new Date(2025, 3, 15, 22, 0),
  ...
  startDate: new Date(2025, 3, 30, 0, 0),
  // April 30, 2025, all day
  endDate: new Date(2025, 3, 30, 23, 59),
  // April 30, 2025, end of day
  allDay: true,
  color: '#EA4335',
  ...
  startDate: new Date(2025, 3, 7, 8, 0),
  // April 7, 2025, 8:00 AM
  endDate: new Date(2025, 3, 7, 9, 0),
...
const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const filteredEvents = events.filter(event => searchTerm ? event.title.toLowerCase().includes(searchTerm.toLowerCase()) || event.description && event.description.toLowerCase().includes(searchTerm.toLowerCase()) || event.location && event.location.toLowerCase().includes(searchTerm.toLowerCase()) : true);
  ...
  const isDayWithEvents = (day: Date) => {
    return filteredEvents.some(event => isSameDay(event.startDate, day) || isSameDay(event.endDate, day));
  };
  ...
  return <div className="space-y-4">
      ...
      <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
        {selectedEvent && <DialogContent className="max-w-md max-h-[90vh]">
            <DialogHeader>
              ...
              backgroundColor: selectedEvent.color
                ...
                <div className="flex space-x-1">
                  <Button variant="outline" size="icon" onClick={handleEditEvent} className="h-8 w-8">
                    <Edit className="h-4 w-4" />
                  </Button>
                  <Button variant="outline" size="icon" onClick={handleDeleteEvent} className="h-8 w-8 text-destructive hover:text-destructive">
                    <Trash className="h-4 w-4" />
                  </Button>
                </div>
              </div>
      ...
      {viewMode === 'month' && <MonthView date={date} setDate={setDate} events={filteredEvents} handleViewEvent={handleViewEvent} theme={theme} weekStartsOn={weekStartsOn} />}
      
      {viewMode === 'week' && <WeekView date={date} setDate={setDate} events={filteredEvents} handleViewEvent={handleViewEvent} theme={theme} weekStartsOn={weekStartsOn} />}
      
      {viewMode === 'day' && <DayView date={date} setDate={setDate} events={filteredEvents} handleViewEvent={handleViewEvent} theme={theme} />}
      
      {viewMode === 'agenda' && <AgendaView date={date} setDate={setDate} events={filteredEvents} handleViewEvent={handleViewEvent} theme={theme} />}
    </div>;
};
